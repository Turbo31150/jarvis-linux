...
**Files:**
- Modify: `/home/turbo/jarvis-linux/canvas/direct-proxy.js:101` (MAX_TOOL_TURNS)
- Modify: `/home/turbo/jarvis-linux/canvas/direct-proxy.js:468-571` (agenticChat)
...
Run: `cd /home/turbo/jarvis-linux && node -e "const dp = require('./canvas/direct-proxy.js'); console.log('loaded OK');"` (ou verifier que le fichier parse sans erreur)
...
```bash
cd /home/turbo/jarvis-linux && git add canvas/direct-proxy.js && git commit -m "feat(cockpit): anti-loop v2 — repetition detection + MAX_TOOL_TURNS 15→8"
```
...
**Files:**
- Modify: `/home/turbo/jarvis-linux/canvas/direct-proxy.js` (nouvelle fonction avant agenticChat, ~ligne 466)
...
```bash
cd /home/turbo/jarvis-linux && node -e "require('./canvas/direct-proxy.js')" 2>&1 | head -5
```
...
```bash
cd /home/turbo/jarvis-linux && git add canvas/direct-proxy.js && git commit -m "feat(cockpit): complexity classifier — simple vs reflexive routing"
```
...
**Files:**
- Modify: `/home/turbo/jarvis-linux/canvas/direct-proxy.js` (nouvelle fonction apres classifyComplexity)
...
```bash
cd /home/turbo/jarvis-linux && git add canvas/direct-proxy.js && git commit -m "feat(cockpit): reflexiveChat — OL1→M1→M2 chain with per-step anti-loop"
```
...
**Files:**
- Modify: `/home/turbo/jarvis-linux/canvas/direct-proxy.js:727-751` (handler POST /chat)
...
```bash
cd /home/turbo/jarvis-linux && git add canvas/direct-proxy.js && git commit -m "feat(cockpit): route /chat uses classifier — simple or reflexive chain"
```
...
**Files:**
- Modify: `/home/turbo/jarvis-linux/canvas/index.html` (CSS + addCockpitMsg)
...
```bash
cd /home/turbo/jarvis-linux && git add canvas/index.html && git commit -m "feat(cockpit): chain badges UI — collapsible reflexive chain display"
```
...
```bash
cp /home/turbo/jarvis-linux/canvas/index.html "C:/Users/franc/Desktop/JARVIS_Cockpit_Portable.html"
```

**Step 2: Redemarrer le proxy**

```bash
taskkill //F //IM node.exe //FI "WINDOWTITLE eq *direct-proxy*" 2>/dev/null; cd /home/turbo/jarvis-linux && node canvas/direct-proxy.js &
```
...
```bash
cd /home/turbo/jarvis-linux && git add -A && git status
