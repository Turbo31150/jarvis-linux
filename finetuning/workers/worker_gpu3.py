...
MODEL = "Qwen/Qwen3-8B"
TRAIN_FILE = r"/home/turbo/jarvis-linux\finetuning\dataset\vocal_chunk_3.jsonl"
EVAL_FILE = None
OUTPUT_DIR = Path(r"F:/BUREAU/turbo/finetuning/output")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
run_name = f"jarvis-qwen3-8b-1660s-vocal3-{timestamp}"
...
def main():
    print(f"\n===== GPU 3 (1660s-vocal3) =====")
    print(f"  Steps: 200 | Seq: 256 | Batch: 1x2 | LoRA r=8")
    ...
    print(f"[OK] LoRA r=8: {t:,} trainables ({100*t/total:.2f}%)")
    ...
    print(f"[>>>] Training GPU 3 (1660s-vocal3) -- 200 steps...")
    ...
    print(f"\n[DONE] GPU 3 (1660s-vocal3): loss={metrics.get('train_loss','?')} runtime={metrics.get('train_runtime',0):.0f}s")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"\n[CRASH] GPU 3 (1660s-vocal3): {type(e).__name__}: {e}")
        traceback.print_exc()
        sys.exit(1)
