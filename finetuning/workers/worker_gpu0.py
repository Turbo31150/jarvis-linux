"""Worker GPU 0 (2060-vocal-resume)"""
...
MODEL = "Qwen/Qwen3-8B"
TRAIN_FILE = r"/home/turbo/jarvis-linux\finetuning\dataset\jarvis_vocal_teaching.jsonl"
EVAL_FILE = None
OUTPUT_DIR = Path(r"/home/turbo/jarvis-linux/finetuning/output")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
run_name = f"jarvis-qwen3-8b-2060-vocal-resume-{timestamp}"
...
def main():
    ...
    print(f"[>>>] Training GPU 0 (2060-vocal-resume) -- 500 steps...")
    ...
    print(f"\n[DONE] GPU 0 (2060-vocal-resume): loss={metrics.get('train_loss','?')} runtime={metrics.get('train_runtime',0):.0f}s")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"\n[CRASH] GPU 0 (2060-vocal-resume): {type(e).__name__}: {e}")
        traceback.print_exc()
        sys.exit(1)
