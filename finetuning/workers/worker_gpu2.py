...
MODEL = "Qwen/Qwen3-8B"
TRAIN_FILE = r"/home/turbo/jarvis-linux\finetuning\dataset\vocal_chunk_2.jsonl"
EVAL_FILE = None
OUTPUT_DIR = Path(r"F:/BUREAU/turbo/finetuning/output")

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
run_name = f"jarvis-qwen3-8b-1660s-vocal2-{timestamp}"
...
def main():
    ...
    print(f"[>>>] Training GPU 2 (1660s-vocal2) -- 200 steps...")
    ...
    print(f"\n[DONE] GPU 2 (1660s-vocal2): loss={metrics.get('train_loss','?')} runtime={metrics.get('train_runtime',0):.0f}s")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"\n[CRASH] GPU 2 (1660s-vocal2): {type(e).__name__}: {e}")
        traceback.print_exc()
        sys.exit(1)
