...
def main():
    ...
    print(f"[>>>] Training GPU 1 (1660s-vocal1) -- 200 steps...")
    ...
    print(f"\n[DONE] GPU 1 (1660s-vocal1): loss={metrics.get('train_loss','?')} runtime={metrics.get('train_runtime',0):.0f}s")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"\n[CRASH] GPU 1 (1660s-vocal1): {type(e).__name__}: {e}")
        traceback.print_exc()
        sys.exit(1)
