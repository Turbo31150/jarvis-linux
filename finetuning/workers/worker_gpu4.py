...
run_name = f"jarvis-qwen3-8b-1660s-vocal4-mix-{timestamp}"
...
def main():
    ...
    print(f"[>>>] Training GPU 4 (1660s-vocal4-mix) -- 200 steps...")
    ...
    print(f"\n[DONE] GPU 4 (1660s-vocal4-mix): loss={metrics.get('train_loss','?')} runtime={metrics.get('train_runtime',0):.0f}s")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        print(f"\n[CRASH] GPU 4 (1660s-vocal4-mix): {type(e).__name__}: {e}")
        traceback.print_exc()
        sys.exit(1)
