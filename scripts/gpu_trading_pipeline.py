...
Usage:
  cd /home/turbo/jarvis-linux && uv run python scripts/gpu_trading_pipeline.py [--cycles N] [--json] [--top N] [--interval S]
"""
import asyncio
import httpx
import json
...
async def run_pipeline(cycles: int = 1, interval: int = 60, top_n: int = 10,
    ...
    for cycle in range(1, cycles + 1):
        ...
        # Sauvegarder en JSON
        if analyses:
            save_path = f"/home/turbo/jarvis-linux/data/pipeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            try:
