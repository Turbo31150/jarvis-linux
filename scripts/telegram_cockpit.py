...
def generate_debug_commands(error: dict) -> list[str]:
    ...
    if "permission" in err or "access denied" in err:
        commands.append(f"icacls /home/turbo/jarvis-linux/data/ 2>&1 | head -5")
    ...
    # Generic task debug
    if not commands:
        commands.append(f"python /home/turbo/jarvis-linux/scripts/task_orchestrator.py --run-once {tid} 2>&1 | tail -20")
