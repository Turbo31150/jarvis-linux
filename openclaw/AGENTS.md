...
- Description newline footgun: write Markdown via heredoc to `/tmp/ghsa.desc.md` (no `"/n"` strings)
...
- Verify: re-fetch; ensure `state=published`, `published_at` set; `jq -r .description | rg '//n'` returns nothing
