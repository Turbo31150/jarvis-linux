...
JarvisCommand("sim_db_backup_all", "pipeline", "Backup toutes les DBs: jarvis + etoile + trading", [
    ...
    ], "pipeline", "powershell:Invoke-RestMethod -Uri 'http://127.0.0.1:1234/api/v1/models/load' -Method Post -Body '{\"model\":\"qwen3-8b\"}' -ContentType 'application/json' -Headers @{'Authorization'='Bearer sk-lm-LOkUylwu:1PMZR74wuxj7OpeyISV7'} -TimeoutSec 60; 'M1 qwen3-8b rechargement lance';;sleep:5;;powershell:$r = Invoke-RestMethod -Uri 'http://127.0.0.1:1234/api/v1/models' -Headers @{'Authorization'='Bearer sk-lm-LOkUylwu:1PMZR74wuxj7OpeyISV7'} -TimeoutSec 5; $loaded = ($r.models | Where-Object {$_.loaded_instances.Count -gt 0}).key -join ', '; \"M1 modeles actifs: $loaded\"", confirm=True),
    ...
    ], "pipeline", "powershell:$q = 'Quelle est la meilleure pratique pour organiser un projet Python avec des modules, tests et config?'; $b1 = @{model='qwen3-8b';messages=@(@{role='user';content=\"/nothink`n$q\"});temperature=0.3;max_tokens=150} | ConvertTo-Json -Depth 3; $r1 = Invoke-RestMethod -Uri 'http://127.0.0.1:1234/v1/chat/completions' -Method Post -Body $b1 -ContentType 'application/json' -Headers @{'Authorization'='Bearer sk-lm-LOkUylwu:1PMZR74wuxj7OpeyISV7'} -TimeoutSec 15; \"[M1/qwen3] $($r1.choices[0].message.content)\";;powershell:$q = 'Quelle est la meilleure pratique pour organiser un projet Python avec des modules, tests et config?'; $b2 = @{model='deepseek-r1-0528-qwen3-8b';messages=@(@{role='user';content=$q});temperature=0.3;max_tokens=150} | ConvertTo-Json -Depth 3; $r2 = Invoke-RestMethod -Uri 'http://192.168.1.26:1234/v1/chat/completions' -Method Post -Body $b2 -ContentType 'application/json' -Headers @{'Authorization'='Bearer sk-lm-keRZkUya:St9kRjCg3VXTX6Getdp4'} -TimeoutSec 30; \"[M2/deepseek-r1] $($r2.choices[0].message.content)\""),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; dbs=[('etoile.db','/home/turbo/jarvis-linux/data/etoile.db'),('jarvis.db','/home/turbo/jarvis-linux/data/jarvis.db')]; [print(f'{n}: {round(os.path.getsize(p)/1024)}KB') or [print(f'  {t[0]}: {sqlite3.connect(p).execute(f/\"SELECT COUNT(*) FROM {t[0]}/\").fetchone()[0]} rows') for t in sqlite3.connect(p).execute(/\"SELECT name FROM sqlite_master WHERE type='table'/\").fetchall()] for n,p in dbs]\" 2>&1 | Out-String"),
    ...
    JarvisCommand("agent_list_status", "pipeline", "Lister les 7 agents Claude SDK et leur status", [
        ...
        "mes agents ia",
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); agents=c.execute('SELECT * FROM agents ORDER BY id').fetchall(); c.close(); [print(f'  {a[1]}: {a[3]} ({a[4]}) — {a[5]}') for a in agents]\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); agents=c.execute('SELECT name, url, model_type, model_name, status, avg_latency FROM agents').fetchall(); c.close(); [print(f'  {a[0]:15} | {a[4]:7} | {a[3]:20} | {a[5] if a[5] else /\"N/A/\"}ms') for a in agents]\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import os,glob; path='/home/turbo/jarvis-linux/finetuning'; files=glob.glob(os.path.join(path,'**/*.jsonl'),recursive=True); cc=lambda f:len(__import__('pathlib').Path(f).read_text(encoding='utf-8').splitlines()); [print(f'  {os.path.basename(f)}: {cc(f)} examples ({round(os.path.getsize(f)/1024)}KB)') for f in files[:10]] if files else print('Aucun dataset JSONL trouve')\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,sys; q='%'; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); r=c.execute('SELECT entity_name,parent,role FROM map WHERE entity_type=/\"skill/\" AND (entity_name LIKE ? OR role LIKE ?) ORDER BY parent LIMIT 20',(q,q)).fetchall(); print(f'{len(r)} skills trouves:'); [print(f'  [{p}] {n}: {d}') for n,p,d in r]; c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:$body = '{\"model\":\"qwen3-8b\",\"input\":\"/nothink/nAnalyse les 3 derniers commits git du projet /home/turbo/jarvis-linux et identifie les patterns de developpement recurrents. Format: PATTERN: description | FREQUENCE: haute/moyenne/basse | ACTION: suggestion\",\"temperature\":0.2,\"max_output_tokens\":512,\"stream\":false,\"store\":false}'; try { $r = Invoke-WebRequest -Uri 'http://127.0.0.1:1234/api/v1/chat' -Method POST -Body $body -ContentType 'application/json' -Headers @{Authorization='Bearer sk-lm-LOkUylwu:1PMZR74wuxj7OpeyISV7'} -TimeoutSec 20 -UseBasicParsing; $d = $r.Content | ConvertFrom-Json; $msg = ($d.output | Where-Object { $_.type -eq 'message' } | Select-Object -Last 1).content; Write-Output \"=== PATTERN LEARNING ===\"; Write-Output $msg } catch { Write-Output 'Pattern learning: M1 offline' }"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,json,datetime; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); rows=c.execute('SELECT category,key,value,confidence FROM memories').fetchall(); data=[{'category':r[0],'key':r[1],'value':r[2],'confidence':r[3]} for r in rows]; out=f'/home/turbo/jarvis-linux/data/memories_export_{datetime.datetime.now().strftime(chr(37)+chr(89)+chr(37)+chr(109)+chr(37)+chr(100))}.json'; __import__('pathlib').Path(out).write_text(json.dumps(data,indent=2,ensure_ascii=False)); print(f'=== EXPORT MEMOIRE ==='); print(f'  {len(data)} entries exportees'); print(f'  Fichier: {out}'); c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); r=c.execute('SELECT category,key,value FROM memories ORDER BY ROWID DESC LIMIT 15').fetchall(); print('=== PATTERNS RECENTS ==='); [print(f'  [{cat}] {k}: {v[:60]}') for cat,k,v in r]; c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); r=c.execute('SELECT entity_name,role FROM map WHERE entity_type=/\"routing_rule/\" ORDER BY entity_name').fetchall(); print(f'=== {len(r)} REGLES ROUTAGE ==='); [print(f'  {n}: {d[:60]}') for n,d in r]; c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; dbs=['/home/turbo/jarvis-linux/data/etoile.db','/home/turbo/jarvis-linux/data/jarvis.db']; print('=== REINDEX DATABASES ==='); [print(f'  {os.path.basename(db)}: ' + (lambda c: (c.execute('REINDEX'), 'REINDEX OK')[1])(sqlite3.connect(db).cursor())) for db in dbs if os.path.exists(db)]\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); tables=c.execute('SELECT name FROM sqlite_master WHERE type=/\"table/\"').fetchall(); print('=== SCHEMA ETOILE.DB ==='); [print(f'  {t[0]}: ' + ', '.join(col[1] for col in c.execute(f'PRAGMA table_info({t[0]})').fetchall())) for t in tables]; c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,shutil,datetime,os; src='/home/turbo/jarvis-linux/data/etoile.db'; ts=datetime.datetime.now().strftime('%Y%m%d_%H%M'); dst=f'/home/turbo/jarvis-linux/data/etoile_snapshot_{ts}.db'; shutil.copy2(src,dst); size=os.path.getsize(dst)/1024; print(f'=== SNAPSHOT ==='); print(f'  Source: etoile.db ({os.path.getsize(src)/1024:.0f}KB)'); print(f'  Snapshot: {os.path.basename(dst)} ({size:.0f}KB)'); print(f'  Date: {ts}')\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; dbs=[('etoile.db','/home/turbo/jarvis-linux/data/etoile.db'),('jarvis.db','/home/turbo/jarvis-linux/data/jarvis.db')]; print('=== SCENARIOS TEST ==='); [print(f'  {name}: ' + str(sqlite3.connect(p).execute('SELECT COUNT(*) FROM sqlite_master WHERE type=/\"table/\"').fetchone()[0]) + ' tables') for name,p in dbs if os.path.exists(p)]; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); tests=c.execute('SELECT COUNT(*) FROM pipeline_tests').fetchone()[0]; maps=c.execute('SELECT COUNT(*) FROM map').fetchone()[0]; print(f'  pipeline_tests: {tests}'); print(f'  map entries: {maps}'); c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); keys=c.execute('SELECT entity_name,role FROM map WHERE entity_type=/\"tool/\" AND role LIKE /\"%%key%%/\" OR role LIKE /\"%%api%%/\"').fetchall(); print('=== API KEYS STATUS ==='); print(f'  Cles referencees: {len(keys)}'); [print(f'  {n}: {r[:50]}') for n,r in keys[:10]]; envs=[f for f in __import__('glob').glob('/home/turbo/jarvis-linux/**/.env',recursive=True)]; print(f'  Fichiers .env: {len(envs)}'); c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:Write-Output '=== CANAUX NOTIFICATION ==='; Write-Output '  [Telegram] @turboSSebot'; $tg = & '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); r=c.execute('SELECT value FROM memories WHERE key=/\"telegram_bot_token/\"').fetchone(); print('OK' if r else 'non configure'); c.close()\" 2>$null; Write-Output \"    Status: $tg\"; Write-Output '  [Console] Toujours actif'; Write-Output '  [TTS] Edge fr-FR-DeniseNeural'; Write-Output '  [Dashboard] port 8080'"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"from src.commands_pipelines import PIPELINE_COMMANDS; import json; data=[{'name':p.name,'category':p.category,'description':p.description,'triggers':p.triggers} for p in PIPELINE_COMMANDS]; print(f'=== EXPORT COMMANDES ==='); print(f'  {len(data)} pipelines exportables'); print(f'  Format: JSON'); __import__('pathlib').Path('/home/turbo/jarvis-linux/data/pipelines_export.json').write_text(json.dumps(data,indent=2,ensure_ascii=False),encoding='utf-8'); print(f'  Fichier: data/pipelines_export.json')\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; dbs=[('etoile.db','/home/turbo/jarvis-linux/data/etoile.db'),('jarvis.db','/home/turbo/jarvis-linux/data/jarvis.db')]; print('=== MERGE CHECK ==='); [print(f'  {n}: {os.path.getsize(p)/1024:.0f}KB, tables=' + str(sqlite3.connect(p).execute('SELECT COUNT(*) FROM sqlite_master WHERE type=/\"table/\"').fetchone()[0]) + ', integrity=' + sqlite3.connect(p).execute('PRAGMA integrity_check').fetchone()[0]) for n,p in dbs if os.path.exists(p)]\" 2>&1 | Out-String"),

    # ══════════════════════════════════════════════════════════════════════
    # BATCH 7 — COMPLETIONS AUDIT: 36 pipelines restantes toutes priorites
    # Fermeture definitive de tous les gaps identifies
    # ══════════════════════════════════════════════════════════════════════

    # --- FINE-TUNING COMPLETIONS (2) ---
    JarvisCommand("finetune_abort_graceful", "pipeline", "Arreter proprement un fine-tuning en cours avec sauvegarde checkpoint", [
        "arreter finetuning", "abort training", "stop finetuning",
        "annuler finetuning", "finetuning stop",
    ], "pipeline", "powershell:Write-Output '=== ABORT FINE-TUNING ==='; $ftProc = Get-Process -Name 'python*' -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -match 'train|finetune|lora' }; if ($ftProc) { Write-Output \"  Training PID: $($ftProc.Id)\"; Write-Output \"  RAM: $([math]::Round($ftProc.WorkingSet64/1MB))MB\"; Write-Output '  Action: Envoyer SIGINT pour checkpoint + arret propre'; Write-Output '  Commande: Stop-Process -Id $($ftProc.Id)' } else { Write-Output '  Aucun training en cours a arreter' }"),
    ...
    JarvisCommand("voice_speaker_profile_switch", "pipeline", "Changer de profil vocal: voix, vitesse, langue", [
        "changer voix", "profil vocal", "switch voice",
        "autre voix", "voice profile",
    ], "pipeline", "powershell:Write-Output '=== PROFILS VOCAUX ==='; Write-Output '  Profil actuel: fr-FR-DeniseNeural (femme)'; Write-Output '  Profils disponibles:'; Write-Output '    fr-FR-DeniseNeural — Femme francaise (defaut)'; Write-Output '    fr-FR-EloiseNeural — Femme jeune'; Write-Output '    en-US-JennyNeural — Femme anglaise'; Write-Output '  Modifier dans: src/voice.py (VOICE_NAME variable)'"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import glob,os; exports=sorted(glob.glob('/home/turbo/jarvis-linux/data/memories_export_*.json'), key=os.path.getmtime, reverse=True); print('=== IMPORT MEMOIRE ==='); print(f'  Backups disponibles: {len(exports)}'); [print(f'  {os.path.basename(f)}: {os.path.getsize(f)/1024:.0f}KB') for f in exports[:5]]; print('  Pour importer: uv run python -c /\"import json,sqlite3; .../\"') if exports else print('  Aucun backup disponible')\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; dbs=[('etoile.db','/home/turbo/jarvis-linux/data/etoile.db'),('jarvis.db','/home/turbo/jarvis-linux/data/jarvis.db')]; print('=== DEFRAG INTENSIVE ==='); [(lambda p,n: (lambda c,s1: (c.execute('VACUUM'), c.execute('ANALYZE'), c.close(), print(f'  {n}: {s1/1024:.0f}KB -> {os.path.getsize(p)/1024:.0f}KB VACUUM+ANALYZE OK')))(sqlite3.connect(p).cursor(), os.path.getsize(p)))(p,n) for n,p in dbs if os.path.exists(p)]\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3,os; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); tests=c.execute('SELECT COUNT(*) FROM pipeline_tests').fetchone()[0]; old_tests=c.execute('SELECT COUNT(*) FROM pipeline_tests WHERE test_date < date(/\"now/\",/\"-30 days/\")').fetchone()[0]; print('=== ARCHIVE DATA ==='); print(f'  Tests totaux: {tests}'); print(f'  Tests >30j: {old_tests} (archivables)'); size=os.path.getsize('/home/turbo/jarvis-linux/data/etoile.db')/1024; print(f'  Taille DB: {size:.0f}KB'); print(f'  Recommandation: {/\"archiver/\" if old_tests > 100 else /\"pas necessaire/\"}'); c.close()\" 2>&1 | Out-String"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); rules=c.execute('SELECT entity_name,role FROM map WHERE entity_type=/\"routing_rule/\"').fetchall(); print('=== BENCHMARK CONSENSUS ==='); print(f'  Regles de routage: {len(rules)}'); [print(f'    {n}: {r[:50]}') for n,r in rules]; weights={'M1':1.8,'M2':1.4,'OL1':1.3,'GEMINI':1.2,'CLAUDE':1.2,'M3':1.0}; print(f'  Poids: {weights}'); print(f'  Quorum: >= 0.65'); c.close()\" 2>&1 | Out-String"),
    ...
    JarvisCommand("model_load_balanced", "pipeline", "Repartir la charge des modeles equilibrement sur les GPU", [
        "load balancing modeles", "equilibrer charge", "balance modeles",
        "repartir gpu", "model load balance",
    ], "pipeline", "powershell:Write-Output '=== LOAD BALANCING ==='; $gpuInfo = & 'nvidia-smi' --query-gpu=index,name,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits 2>$null; if ($gpuInfo) { $gpuInfo | ForEach-Object { $p = $_ -split ','; $used = [int]$p[2].Trim(); $total = [int]$p[3].Trim(); $pct = [math]::Round($used/$total*100); $status = if ($pct -gt 80) { 'SURCHARGE' } elseif ($pct -gt 50) { 'EQUILIBRE' } else { 'DISPONIBLE' }; Write-Output \"  GPU$($p[0].Trim()) $($p[1].Trim()): $pct%% VRAM [$status]\" } } else { Write-Output '  nvidia-smi non disponible' }"),

    JarvisCommand("model_offload_aggressive", "pipeline", "Decharger agressivement les modeles non utilises des GPU", [
        "decharger modeles", "offload gpu", "liberer gpu",
        "model offload", "vider gpu",
    ], "pipeline", "powershell:Write-Output '=== OFFLOAD MODELES ==='; $gpuInfo = & 'nvidia-smi' --query-gpu=index,name,memory.used,memory.free --format=csv,noheader,nounits 2>$null; if ($gpuInfo) { $gpuInfo | ForEach-Object { $p = $_ -split ','; Write-Output \"  GPU$($p[0].Trim()): $($p[2].Trim())MB utilise, $($p[3].Trim())MB libre\" } }; Write-Output '  Pour offload: lms unload <model> via LM Studio CLI'; Write-Output '  Ou: curl -X DELETE http://127.0.0.1:1234/api/v1/models/<id>'"),
    ...
    JarvisCommand("n8n_execution_history_clear", "pipeline", "Nettoyer l'historique d'executions n8n", [
        ...
        "purger n8n", "n8n history clear",
    ], "pipeline", "powershell:Write-Output '=== CLEAR N8N HISTORY ==='; try { $r = Invoke-WebRequest -Uri 'http://127.0.0.1:5678/healthz' -TimeoutSec 3 -UseBasicParsing; Write-Output \"  n8n: actif ($($r.StatusCode))\"; Write-Output '  Purger via: n8n UI > Settings > Executions > Prune' } catch { Write-Output '  n8n: offline (port 5678)'; Write-Output '  Demarrer n8n puis purger via UI' }"),
    ...
    ], "pipeline", "powershell:& '/\Users/franc/.local/bin/uv.exe' run python -c \"import sqlite3; c=sqlite3.connect('/home/turbo/jarvis-linux/data/etoile.db'); keys=c.execute('SELECT entity_name,role FROM map WHERE entity_type=/\"tool/\" AND (role LIKE /\"%%key%%/\" OR role LIKE /\"%%token%%/\" OR role LIKE /\"%%api%%/\")').fetchall(); print('=== ROTATION CLES API ==='); print(f'  Cles referencees: {len(keys)}'); [print(f'  {n}: {r[:50]}') for n,r in keys[:10]]; print('  Recommandation: rotation tous les 90 jours'); c.close()\" 2>&1 | Out-String"),
    ...
    JarvisCommand("workspace_session_persist", "pipeline", "Persister la session actuelle pour reprise apres redemarrage", [
        ...
        "sauvegarder session", "persist workspace",
    ], "pipeline", "powershell:Write-Output '=== PERSIST SESSION ==='; $procs = Get-Process | Where-Object { $_.MainWindowTitle -and $_.MainWindowTitle -ne '' } | Select-Object ProcessName,MainWindowTitle; Write-Output \"  Fenetres ouvertes: $($procs.Count)\"; $procs | Select-Object -First 8 | ForEach-Object { Write-Output \"    $($_.ProcessName): $($_.MainWindowTitle.Substring(0, [Math]::Min(40, $_.MainWindowTitle.Length)))\" }; Write-Output '  Pour persister: Sauvegarder la liste dans un fichier'; Write-Output '  Restaurer: Relancer les apps au demarrage'"),
    ...
    JarvisCommand("notification_frequency_optimize", "pipeline", "Optimiser la frequence des notifications pour eviter le spam", [
        "frequence notifications", "optimiser alertes", "moins notifications",
        "notification frequency", "reduce alerts",
    ], "pipeline", "powershell:Write-Output '=== FREQUENCE NOTIFICATIONS ==='; Write-Output '  Niveaux actuels:'; Write-Output '    CRITICAL: immediate (GPU >85C, noeud down)'; Write-Output '    WARNING: toutes les 5min max (GPU >75C, RAM >90%%)'; Write-Output '    INFO: toutes les 30min max (routine, stats)'; Write-Output '    DEBUG: desactive en prod'; Write-Output '  Canaux: Console (toujours), TTS (si voice), Telegram (si configure)'"),
